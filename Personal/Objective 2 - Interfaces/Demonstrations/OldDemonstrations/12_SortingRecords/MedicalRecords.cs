...
{
    internal class MedicalRecord : IComparable
    {
        ...
        internal MedicalRecord(int id, string name, string address, string phone, string physician) : this()
        {
            _patientId = id;
            _patientName = name;
        ...
        int IComparable.CompareTo (object o)
        {
            ...
            if (null != mr)
            {
                // The property PatientId returns an integer. The System.Int32
                // type supports a CompareTo() method as well so we'll just
                // use it to perform the comparison.
                result = this.PatientId.CompareTo(mr.PatientId);
            }
            else
            {
                Console.WriteLine ("\nMEDICALRECORD: Object passed is in not a MedicalRecord object - cannot compare.");
            }
        ...
        internal int PatientId
        {
            ...
            set
            {
                if (value <= 0)
                {
                    Console.WriteLine ("\nMEDICALRECORD - Invalid patient ID of {0} entered.", value);
                }
                else
                {
                    _patientId = value;
        ...
        internal string PrimaryCarePhysician
        {
            ...
            set
            {
                if (value.Length <= 0)
                {
                    Console.WriteLine ("\nMEDICALRECORD - Invalid patient primary care physician.");
                }
                else
                {
