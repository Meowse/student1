...
namespace SortingRecords
{
    internal class DataGenerator
    {
        ...
        internal DataGenerator()
        {
            if (null == randomizer)
            {
                // Create a random number generator for the list elements. The
                // seed is based on the millisecond value of the current time
        ...
        internal string GetString(int size, bool bCapitalizeFirstLetter)
        {
            ...
            // Check to make sure the size of the array being passed in is valid.
            if (bSuccess)
            {
                if (size <= 0)
                {
            ...
            if (bSuccess)
            {
                ...
                {
                    s = s + (Convert.ToChar(GetRandomNumber(97, 122))).ToString();
                }
            }
        ...
        {
            bool bSuccess = true;   // Use a flag to force a single exit point.
            int result = -1;
            ...
            if (bSuccess)
            {
                if (minValue <= 0)
                {
                    Console.WriteLine ("\nDATAGENERATOR: The minimum value provided must be greater than 0.");
                    bSuccess = false;
                }
            }
            ...
            if (bSuccess)
            {
                ...
                {
                    Console.WriteLine ("\nDATAGENERATOR: The minimum value {0} has a greater magnitude " +
                        "than the maximum value of {1}.", minValue, maxValue);
                    bSuccess = false;
                }
            }
            ...
            if (bSuccess)
            {
                ...
                if (result < 0)
                {
                    Console.WriteLine ("\nDATAGENERATOR: Result of {0} will be converted to a positive number.",
                        result);
