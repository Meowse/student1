 ...
 * 2. Using block for processing a type that implements the
...
//
// NOTE: Set a breakpoint in the public Dispose()
//       method of Customer.
...
internal class Customer : IDisposable
{
    ...
    // Private cleanup method that is called from the Dispose method. 
    //  The disposed flag is checked to make sure cleanup occurs only 
    //  1 time. The argument passed in is checked to see if any objects 
    ...
    // Class Dispose() method. This method is declared in the IDisposable 
    //  interface, and implemented here.
    public void Dispose ()
    {
        Cleanup(true);
    }
