/************************************************************************
 * 
 * Interfaces: Part 4 (Program.cs)
 * 
 * Core Topics:
 * 1. Explicit implementation of interfaces.
 * 2. Resolving ambiguity among different interface members.
 **********************************************************************/
...
namespace InterfaceDemo
{
    ...
    // By implementing IDiscoverable, there is a naming ambiguity
    // between it and ISearchable. No warning or error occurs, but
    // we should indicate to the compiler and client code what our
    // intentions are.
    internal class Collection : ISearchable, ISortable, IDiscoverable
    {
        ...
        // We are explicitly saying that this method applies to the one
        // identified in ISearchable. Notice we had to remove the access
        // modifier. The compiler doesn't like it. Also notice that if
        // you compile without adding a Found() method for 
        // IDiscoverable, the compiler generates an error.
        bool ISearchable.Found(object o)
        {
            bool found = false;
            int index = 0;
            int targetData = 0;
        ...
        bool IDiscoverable.Found(object o)
        {
            bool result = false;

            ISearchable searchableObject = this as ISearchable;
            if (null != searchableObject)
            {
                result = searchableObject.Found(o);
            }
        ...
        public void Sort ()
        {
            ...
            // rather than int, we would have to implement 
            // IComparable to implement how to compare two objects.
            ...
            while (keepGoing)
            {
                ...
                switch (userOption)
                {
                    ...
                    case "2":
                        ...
                        if (targetNumber > 0)
                        {
                            IDiscoverable discoverableList = 
                                listOfNumbers as IDiscoverable;

                            if (null != discoverableList)
                            {
                                if (discoverableList.Found(targetNumber))
                                {
                                    Console.WriteLine
                                        ("\n{0} is in the list.\n",
                                        targetNumber);
