...
namespace UsingAsyncDelegates
{
    ...
    class Program
    {
        ...
        internal void ComputationComplete(IAsyncResult plainResult)
        {
            AsyncResult ar = (AsyncResult)plainResult;
            CalculateMethod operation = (CalculateMethod)ar.AsyncDelegate;
            TotalValue = operation.EndInvoke(plainResult);
        ...
        static void Main()
        {
            ...
            AsyncCallback ac = new AsyncCallback(p.ComputationComplete);

            // Call the Add method stored in the delegate asynchronously.
            //TODO:  Include the instance of the callback delegate in the
            //      call below.
            IAsyncResult asyncResult = cm.BeginInvoke(firstNumber, secondNumber, ac, null);
