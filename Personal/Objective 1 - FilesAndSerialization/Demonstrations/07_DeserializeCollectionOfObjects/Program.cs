/************************************************************************
 * 
 * DeserializeCollectionOfObjects: Part 7
 * 
 * Core Topics:
 * 1. Deserialize method of the FormatterBinary class for deserializing  
...
namespace Serialization
{
    internal class Employees
    {
        ...
        internal void SaveEmployeeCollection(DirectoryInfo employeeDirectory)
        {
            Stream employeeCollectionFile;
            BinaryFormatter binFormat = new BinaryFormatter();
            string directoryFullName = employeeDirectory.FullName;
            string employeesFullFileName;

            employeesFullFileName = directoryFullName +
                 @"\Employees.dat";

            // using block guarantees that the opened file will be closed when 
            //      the block is done.
            using (employeeCollectionFile = 
                new FileStream(employeesFullFileName, FileMode.Create,
                    FileAccess.Write, FileShare.None))
            {
                binFormat.Serialize(employeeCollectionFile, employees);
            }
        ...
        internal void GetSavedEmployees(DirectoryInfo employeeDirectory)
        {
            BinaryFormatter binFormat = new BinaryFormatter();
            FileInfo[] employeeFiles = employeeDirectory.GetFiles("*.dat");
            Stream employeeFile;

            foreach (FileInfo employeeFileInfo in employeeFiles)
            {
                using (employeeFile = File.OpenRead(employeeFileInfo.FullName))
                {
                    employees = (List<Employee>)binFormat.Deserialize(employeeFile);
                }
            }
        }
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // Load collection with employees.
            if (Directory.Exists(_EMPLOYEE_FILES_DIRECTORY_FULL_NAME))
            {
                // Load collection with previously saved employees.
                employeeDirectory = 
                    new DirectoryInfo(
                            _EMPLOYEE_FILES_DIRECTORY_FULL_NAME);
                emps.GetSavedEmployees(employeeDirectory);
            }
            else
            {
                // Create new instances of employees.

                // Create an instance of the directory for saving employees later on.
                employeeDirectory = 
            ...
            emps.SaveEmployeeCollection(employeeDirectory);
