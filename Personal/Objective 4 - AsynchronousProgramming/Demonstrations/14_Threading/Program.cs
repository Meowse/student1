 ...
 * Part 14: Threading
 * 
 * Topic:  Setting thread priorities.  Take note that the second secondary
 *      thread may not be showing as red.  That does mean a possible bug
 *      in this multi-threaded application.
 ***************************************************************************/
...
namespace ThreadsDemo
{
    ...
    class Program
    {
        ...
        private static bool JoinThread(int mainThreadId, Thread thread)
        {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                // Set the priority of the second thread to be quite a 
                // higher than the other two threads. Even though we
                // start the first thread before the second thread, the
                // second thread gets a higher priority and calculates
                // its value first.
                secondaryThread2.Priority = ThreadPriority.Highest;
                secondaryThread1.Priority = ThreadPriority.Lowest;
                ...
                if (JoinThread(threadId, secondaryThread1))
                {
                ...
                if (JoinThread(threadId, secondaryThread2))
                {
                    Console.WriteLine("\n{0}: The result from {1} is: {2}",
