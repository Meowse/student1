 ...
 * Part 16B: Threading
 * 
 * Topic:  1.   Use the TryEnter method of the Monitor class with a singleton 
 *                  object to control the serial
 *                  use of a common resource (Console window in this example)
 *                  by multiple threads to avoid collisions.
 *         2.   Illustrate how to control infinite loops inside of a block
 *                  controlled by the Monitor class.
 ***************************************************************************/
...
namespace ThreadsDemo
{
    ...
    internal class ComplicatedCalculator
    {
            ...
            (double firstNumber, double secondNumber)
        {
            ...
            if (Monitor.TryEnter (SyncObject.Sync,2000))
            {
                try
                {
                    ...
                    Program.LockingThread = Thread.CurrentThread;
    ...
    class Program
    {
        private static Thread _lockingThread = null;
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                if (Program.LockingThread != null)
                {
                    JoinThread(threadId, LockingThread);
                }
        ...
        public static Thread LockingThread
        {
            get { return _lockingThread; }
            set { _lockingThread = value; }
        }
    }
}
