 ...
 * Part 17: Threading
 * 
 * Topic:  1.   Uses the thread pool to execute background secondary
 *                      threads.
 ***************************************************************************/
...
namespace ThreadsDemo
{
    ...
    class Program
    {
        ...
        public static void ThreadDoneCallback()
        {
            lock (SyncObject.Sync)
            {
                // The Interlocked class provide atomic operations for 
                // things like incrementing, decrementing, and assignment.
                // So synchronization occurs in this class.
        ...
        static void Main(string[] args)
        {
            ...
            // Set the original color of the console. 
            ...
            try
            {
                ...
                // Start up a set of ThreadPool threads. The second argument
                // to QueueUserWorkItem() is passed as an object. 
                // CalculateValue() will have to cast the object back to two 
                // doubles.
                for (int i = 0; i < ccList.Length; i++)
                {
                    double[] numbers = 
                    ...
                    if (!ThreadPool.QueueUserWorkItem
                        (ccList[i].CalculateValue, numbers))
                    {
                        lock (SyncObject.Sync)
                        {
                            Console.WriteLine("\n{0}: ERROR in queuing a thread.",
                                threadId);
                        }
                    }
                }
