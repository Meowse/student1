...
namespace AsynchSorting
{
    ...
    internal class SortAndSearch
    {
        ...
        internal delegate void MethodToCall();
        ...
        internal void SortCallbackMethod(IAsyncResult iar)
        {
            try
            {
                // Obtain the delegate from the state object.
                MethodToCall MTC = (MethodToCall) iar.AsyncState;
            ...
            {
                // If an exception is thrown in EndInvoke(), catch 
                // it here and display the cause.
        ...
        static void Main()
        {
            ...
            try
            {
                ...
                if (size > 0)
                {
                    ...
                    WaitHandle[] waiter = new WaitHandle[2];
                    ...
                    SortAndSearch SAS1 = new SortAndSearch (size, num, SortAndSearch.SortType.InsertionSort);
                    waiter[0] = SAS1.CallSortAndSearchCallback (new SortAndSearch.MethodToCall(SAS1.SortingMethod));
                    ...
                    SortAndSearch SAS2 = new SortAndSearch (size, num, SortAndSearch.SortType.HeapSort);
                    waiter[1] = SAS2.CallSortAndSearchCallback (new SortAndSearch.MethodToCall(SAS2.SortingMethod));

                    Console.WriteLine ("I'm going to wait now...\n");

                    // TODO: Wait for all the handles indefinitely.
                    WaitHandle.WaitAll (waiter);
                }
            }
            catch (Exception e)
            {
